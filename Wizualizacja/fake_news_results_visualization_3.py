...
def compare_vects(df, vects, ds_name):
    df = df[df.classifier != "DummyClassifier15"]

    for indx, g in enumerate(vects):

        res = pd.DataFrame(
            columns=['vec1', 'vec2', 'mean1', "mean2", 'var1', 'var2', 'f', 'p-value'])
        for i in range(len(g)):
            for j in range(len(g)):
                mean1, mean2, var1, var2, f, t = compare_results(
                    df, None, None, g[i], g[j], 'accuracy')
                res = res.append({
                    'vec1': g[i],
                    'vec2': g[j],
                    'mean1': mean1,
        ...
        #                            index=["vec2"], columns=["vec1"])

        # df_styled = df_styled['f'].rename(index=vects_new_names, level=0)
        # # print(df_styled)
        # # print(df_styled.columns)
        # df_styled = df_styled.rename(columns=vects_new_names, level='vec1')

        # # print(df_styled)
        # df_styled = df_styled.style.applymap(
        #     lambda v: 'color:green;' if (v > 1) else None).format(formatter="{:.4g}")
        # dfi.export(df_styled, "vects_f_{}_{}_all.png".format(indx, ds_name))

        df_styled2 = pd.pivot_table(res, values=['p-value'],
                                    index=["vec2"], columns=["vec1"])

        df_styled2 = df_styled2['p-value'].rename(
            index=vects_new_names, level=0)
        df_styled2 = df_styled2.rename(columns=vects_new_names, level='vec1')

        # print(df_styled2)
        df_styled2 = df_styled2.style.applymap(
            lambda v: 'color:green;' if (v < 0.05) else None).format(formatter="{:.4g}")
        # dfi.export(df_styled2, "vects_p_{}_{}.png".format(indx, ds_name))
...
def main(argv):
    ...
    for ds_name in datasets:
        ...
        # compare_vects(df, [['TfidfVectorizer1', 'TfidfVectorizer6', 'CountVectorizer13', 'CountVectorizer16', 'NoneType20']], ds_name)
        ...
        compare_clfs(df, [['SVC1', 'DecisionTreeClassifier4', 'RandomForestClassifier6', 'BernoulliNB14']], ds_name)

        # res = pd.DataFrame(
        #     columns=['Klasyfikator', 'Średnia', 'Wariancja'])

        # for indx, g in enumerate([item for sublist in clfs for item in sublist]):
        #     a1 = df.loc[df['classifier'] == g]['accuracy']
        #     res = res.append({
